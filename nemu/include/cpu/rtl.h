...
static inline void rtl_push(const rtlreg_t* src1) {
  // esp <- esp - 4
  // M[esp] <- src1
  rtl_subi(&cpu.esp,&cpu.esp,4);//进桟做减法
...
static inline void rtl_pop(rtlreg_t* dest) {
  ...
  rtl_addi(&cpu.esp,&cpu.esp,4);//出桟做加法
}
...
static inline void rtl_msb(rtlreg_t* dest, const rtlreg_t* src1, int width) {
  // dest <- src1[width * 8 - 1]
  //TODO();
  rtl_shri(dest,src1,width*8-1);//右移到该位上
  rtl_andi(dest,dest,0x1);//取最后一位
}

static inline void rtl_update_ZF(const rtlreg_t* result, int width) {
  // eflags.ZF <- is_zero(result[width * 8 - 1 .. 0])
  //TODO();
  assert(result!=&t0);//临时寄存器t0的地址不能和result相同，否则会改变result
  rtl_andi(&t0,result,(0xffffffffu >> (4-width)*8));//取result的后width字节
  rtl_eq0(&t0,&t0);//判断是否为0,是则翻转为1
  rtl_set_ZF(&t0);
}
...
static inline void rtl_update_SF(const rtlreg_t* result, int width) {
  ...
  //TODO();
  assert(result!=&t0);
  rtl_msb(&t0,result,width);//使用rtl_msb函数取result[width*8-1]这一位并判断0/1
  rtl_set_SF(&t0);
}
...
static inline void rtl_load_cr(rtlreg_t* dest,int r) {
  switch(r) {
    case 0:*dest=cpu.CR0;return;//访问CR0
    case 3:*dest=cpu.CR3;return;//访问CR3
    default:assert(0);
  }
  return;
}
...
static inline void rtl_store_cr(int r,const rtlreg_t* src) {
  switch(r) {
    case 0:cpu.CR0=*src;return;//修改CR0
    case 3:cpu.CR3=*src;return;//修改CR3
    default:assert(0);
  }
  return;
}
